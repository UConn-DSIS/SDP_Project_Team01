...
def configure_models(time, target, prediction_length, num_windows, datatype, filename, chronos_model, delimiter):
    pipeline = BaseChronosPipeline.from_pretrained(
        f"amazon/{chronos_model}",  # use "amazon/chronos-bolt-small" for the corresponding Chronos-Bolt model
    ...
    return forecast(pipeline, df, time, target, filename, prediction_length, num_windows, chronos_model, delimiter)

def forecast(pipeline, df, time, target, filename, prediction_length, num_windows, chronos_model, delimiter):
    # partition df into sliding windows
    df_length = len(df[target])

    # at most, num_windows=df_length (1 window for each singular data point)
    if num_windows > df_length:
        num_windows = df_length

    step = df_length//num_windows
    # evenly space out starting points across df
    ...
    for i in range(len(starts)):
        ...
        context = []
        for val in df[target][starts[i]:ends[i]].values:
            context.append(int(val))
    ...
    return post(filename, time, target, start_time, end_time, chronos_model)

def post(filename, time, target, start_time, end_time, chronos_model):
    grafana_url = "http://localhost:3000"

    # TODO: set username and password for local Grafana
    ...
    # TODO: replace token field with the one you generated
    # token from service account for privileged groups to see
    token = "" 
    uid = create_dashboard_response.json()['uid']  # get actual UID
